...
| CloudWatch      | 1 dashboard + 28 alarms + 1 composite | API Gateway, Lambda (incl. chat), Cognito triggers, Step Functions chat workflow, AppSync Events; composite `Chat-Unhealthy`     |
...
**Run-rate ≈ $5–6 / month** after the July 2026 alarm cleanup. Breakdown of
the drivers:

| Service                                                            | ~$/month | Note                                                |
| ------------------------------------------------------------------ | -------- | --------------------------------------------------- |
| CloudWatch                                                         | ~3.40    | **28 alarms + 1 composite = $3.30** + log ingestion |
...
Alarms were reduced from 34 to 23 in July 2026 (28 after adding the receipt-analysis and image-normalization Lambdas) by dropping the 10 per-Lambda
...
The remaining cost levers, in order: **CloudWatch alarms + log ingestion**
(~60% of the bill), then **Step Functions log level** (stage-aware: `ERROR` in
prod) and **Bedrock tokens** — the two that grow with traffic. Note that
_consolidating_ alarms saves nothing (metric math bills per metric referenced);
only removing monitored metrics does.
