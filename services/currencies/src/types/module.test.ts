...
import { ROUTES } from '@services/currencies/presentation/router';
...
describe('ModuleType contract', () => {
  ...
  it('controller factory for /currencies creates a controller with the provided app', () => {
    const app = makeApp();
    const route = ROUTES.find((r) => r.url === '/currencies');
    if (!route) throw new Error('/currencies route not found');
    const ctrl = route.controller(app);
