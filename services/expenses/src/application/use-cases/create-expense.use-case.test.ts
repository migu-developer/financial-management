...
describe('CreateExpenseUseCase', () => {
  ...
  it('passes null global_value when conversion returns null', async () => {
    const repository = makeMockRepository();
    const conversionService = makeMockConversionService();
    repository.create.mockResolvedValue({ ...mockExpense, global_value: null });
    conversionService.convert.mockResolvedValue(null);

    const useCase = new CreateExpenseUseCase(repository, conversionService);
    await useCase.execute(input, 'uid-1', 'u@test.com');
  ...
  it('passes date from input through to repository', async () => {
    ...
    const useCase = new CreateExpenseUseCase(repository, conversionService);
    await useCase.execute(inputWithDate, 'uid-1', 'u@test.com');
