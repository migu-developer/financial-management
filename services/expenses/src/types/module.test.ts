...
describe('ModuleType contract', () => {
  ...
  it('controller factory for /expenses creates a controller with the provided app', () => {
    const app = makeApp();
    const route = ROUTES.find((r) => r.url === '/expenses');
    if (!route) throw new Error('/expenses route not found');
    const ctrl = route.controller(app);
    expect(ctrl.app).toBe(app);
  });

  it('controller factory for /expenses/{id} creates a controller with the provided app', () => {
    const app = makeApp();
    const route = ROUTES.find((r) => r.url === '/expenses/{id}');
    if (!route) throw new Error('/expenses/{id} route not found');
    const ctrl = route.controller(app);
    expect(ctrl.app).toBe(app);
  });
