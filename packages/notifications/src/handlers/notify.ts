...
const ses = tracerService.captureAWSv3Client(new SESClient({}));
const s3 = tracerService.captureAWSv3Client(new S3Client({}));
...
export async function handler(event: SNSEvent): Promise<void> {
  ...
  for (const record of event.Records) {
    try {
      ...
      tracerService.putAnnotation('alarmName', payload.alarmName);
      tracerService.putAnnotation('severity', payload.severity);
