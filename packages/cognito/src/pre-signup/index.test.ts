...
jest.mock('@pre-signup/infrastructure/adapters/cognito-admin.adapter', () => ({
...
describe('pre-signup handler', () => {
  it('dispatches to trigger handler for ExternalProvider', async () => {
    ...
    expect(mockTriggerHandler).toHaveBeenCalledWith(event, expect.anything());
    expect(result).toBe(event);
  });

  it('skips trigger handler for native SignUp', async () => {
  ...
  it('skips trigger handler for AdminCreateUser', async () => {
    const event = buildEvent('PreSignUp_AdminCreateUser', 'admin-user');
    await handler(event);
    expect(mockTriggerHandler).not.toHaveBeenCalled();
  });
