...
describe('migration v2.0.0', () => {
  ...
  it('up script creates tables with RLS enabled', () => {
  ...
  it('up script grants access to readonly_lambda_role', () => {
    const script = versionConfig.scripts[0]!;
    if (script.type !== 'sql') return;
    const sql = fs.readFileSync(
      path.join(migrationDir, `${script.up}.sql`),
    ...
    expect(sql).toContain('GRANT readonly_lambda_role TO readonly_lambda');
  });
